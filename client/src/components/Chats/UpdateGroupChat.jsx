...
function UpdateGroupChat({
  ...
  async function handleRename() {
    ...
    try {
      const { data } = await axios.put(
        `${process.env.REACT_APP_SERVER_URL}chat/group/rename`,
        {
  ...
  async function handleSearch(query) {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}user/allusers?search=${search}`,
        config
      );
      setSearchData(data);
      setLoading(false);
  ...
  async function handleAddUser(userToadd) {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_SERVER_URL}chat/group/add`,
        {
          chatid: selectedChat._id,
  ...
  async function handleRemove(u) {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${process.env.REACT_APP_SERVER_URL}chat/group/remove`,
        {
          chatid: selectedChat._id,
          userid: u._id,
        },
      ...
      if (data) {
        setSelectedChat((prevChat) => {
          const updatedUsers = prevChat.users.filter(
            (user) => user._id !== u._id
          );
          return {
            ...prevChat,
      ...
      >
        ...
        <ModalContent>
              ...
              searchData
                ...
                .map((searchUser) => (
                  <UserList
                    ...
                    handleFunc={() => handleAddUser(searchUser)}
                  />
                ))
            )}
          </ModalBody>
          <ModalFooter>
            <Button
              onClick={() => {
                handleRemove(selectedChat.groupAdmin);
                onClose();
              }}
              colorScheme="red"
            >
