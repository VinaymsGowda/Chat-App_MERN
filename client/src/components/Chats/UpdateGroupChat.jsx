...
function UpdateGroupChat({children,fetchAgain,setFetchAgain,fetchMessages}) {
    ...
    async function handleRename(){
        ...
        try {
        const {data}=await axios.put("http://localhost:4000/chat/group/rename",
        {
...
async function handleAddUser(userToadd) {
  ...
  try {
    ...
    const response = await axios.put(
      "http://localhost:4000/chat/group/add",
      {
        chatid: selectedChat._id,
...
async function handleRemove(u){
  ...
  try {
    ...
    const { data } = await axios.put("http://localhost:4000/chat/group/remove", {
      chatid: selectedChat._id,
      userid: u._id,
    }, config);
